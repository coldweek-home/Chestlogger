...
void oledm_start_(struct OLEDM* display) {
	_reset_globals();
	oledm_ifaceInit();
...
void oledm_start_good_contrast(struct OLEDM* display) {
	_reset_globals();
	oledm_ifaceInit();
	...
	oledm_startCommands(err);
	oledm_command(SH1106_DISPLAYOFF, err);         // 0xAE

	oledm_command(SH1106_SETDISPLAYCLOCKDIV, err); // 0xD5
	oledm_command(0x80, err);

	oledm_command(SH1106_SETMULTIPLEX, err);       // 0xA8
	oledm_command(0x3F, err);                      // 64 rows

	oledm_command(SH1106_SETDISPLAYOFFSET, err);   // 0xD3
	oledm_command(0x00, err);

	oledm_command(SH1106_SETSTARTLINE | 0x00, err); // 0x40

	//oledm_command(0x30 | 0x03, err);               // pump voltage 9V

	//oledm_command(SH1106_SEGREMAP | 0x01, err);    // 0xA1
	oledm_command(SH1106_SEGREMAP, err);    // 0xA0

	//oledm_command(SH1106_COMSCANDEC, err);         // 0xC8
	oledm_command(SH1106_COMSCANINC, err);

	oledm_command(SH1106_SETCOMPINS, err);         // 0xDA
	oledm_command(0x12, err);                      // alternate COM config was 0x12 or 0x02

	oledm_command(SH1106_SETVCOMDETECT, err);      // 0xDB
	oledm_command(0x40, err);



	oledm_command(SH1106_SETPRECHARGE, err);       // 0xD9
	oledm_command(0xF1, err);
	
	//Contrast issues:
	//oledm_command(0xAD, err);                      // DC/DC control
	//oledm_command(0x8A, err);                      // DC/DC on
		
	oledm_command(0x8D, err);  // SSD1306 charge pump
	oledm_command(0x14, err);
		
	oledm_command(SH1106_SETCONTRAST, err);
	oledm_command(0xFF, err);
	
	oledm_command(SH1106_DISPLAYALLON_RESUME, err); // 0xA4
	oledm_command(SH1106_NORMALDISPLAY, err);       // 0xA6
	oledm_command(SH1106_DISPLAYON, err);           // 0xAF
...
void oledm_start(struct OLEDM* display) {
	...
	oledm_command(SH1106_SETDISPLAYOFFSET, err);   // 0xD3
	oledm_command(0x20, err);

	oledm_command(SH1106_SETSTARTLINE | 0x00, err); // 0x40

	
	oledm_command(0xAD, err);                      // DC/DC control
	oledm_command(0x8B, err);                      // DC/DC on
	...
	oledm_command(SH1106_SETCONTRAST, err);
	oledm_command(0x7F, err);


	oledm_command(SH1106_SETPRECHARGE, err);       // 0xD9
	oledm_command(0x22, err);
...
void oledm_clear(struct OLEDM* display, uint8_t byte) {
  ...
  for (; row < 8; ++row) {
    ...
    oledm_command(SH1106_SETLOWCOLUMN  | 0x02, err);  // start at col 2 (hardware offset)
    oledm_command(SH1106_SETHIGHCOLUMN | 0x00, err);  // high nibble = 0
    oledm_stop(display);
