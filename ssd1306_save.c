...
void oledm_basic_init(struct OLEDM* display) {
  ...
  display->option_bits = 0; //OLEDM_ALTERNATE_COM_CONFIG | OLEDM_ROTATE_180;
}
...
void oledm_start(struct OLEDM* display) {
  ...
  if (display->option_bits & OLEDM_ROTATE_180){
	oledm_command(SSD1306_COMSCANINC, err);
  }
  else {
  ...
  // For proper left-to-right display, the SSD1306_SEGREMAP
  // needs to be revered from default.
  //oledm_command(SSD1306_SEGREMAP | 0x01, err);
  if (display->option_bits & OLEDM_ROTATE_180){
	oledm_command(SSD1306_SEGREMAP, err);
  }
  else {
    oledm_command(SSD1306_SEGREMAP | 0x01, err);
  }
  ...
  oledm_command(SSD1306_SETMULTIPLEX, err);
  oledm_command(0x3F, err);  // 64 rows - 1
  
  oledm_command(SSD1306_SETDISPLAYOFFSET, err);
  oledm_command(0x00, err);
...
void oledm_rotate(struct OLEDM* display){
  error_t* err = &(display->error);
//  oledm_startCommands(err);
//  oledm_command(0xA1, err);  // Mirror horizontally
//  oledm_stop(display);
  oledm_startCommands(err);
  oledm_command(0xC8, err);  // Mirror vertically
  oledm_stop(display);
